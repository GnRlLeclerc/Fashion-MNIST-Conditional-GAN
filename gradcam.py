...
class GradCAM:
    ...
    def _save_gradients_hook(self, _module: nn.Module, _grad_input: tuple[Tensor], grad_output: tuple[Tensor]):
        self.gradient = grad_output[0].detach().cpu().numpy()

    def _save_activations_hook(self, _module: nn.Module, _grad_input: tuple[Tensor], output: Tensor):
